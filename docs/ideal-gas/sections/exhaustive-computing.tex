...
\textbf{Window $t_1$: Observing $\alpha$:} Left diagram shows observation window (blue circle $\alpha$) isolated from $\beta$ and $\gamma$ (dashed lines). S-entropy coordinates $\mathbf{S} = [1.00, 1.00, 0.91]$. 
\textbf{Window $t_3$: $\beta + \gamma$:} Left diagram shows window shifted to include $\beta$ and $\gamma$ (orange), excluding $\alpha$. S-entropy coordinates $\mathbf{S} = [1.00, 1.00, 0.94]$. 
\textbf{Window $t_4$: Cycle Complete:} Left diagram shows window returned to original configuration (all three molecules visible).}
...
\begin{figure*}[!htbp]
\centering
\includegraphics[width=0.9\textwidth]{figures/exhaustive_computing_panel.png}
...
\textbf{(F)} Memory by existence: Trajectory visualization in a 2D projection of S-entropy space, with points colored by visit order, demonstrates that memory emerges from the exploration history without explicit storage.}
